---
title: Redis源码-快速列表quicklist
date: 2020-03-30 11:57:53
tags: Redis
categories: 
- Redis
- 1-data-structure
...
Redis3.2版本以前的列表底层的编码是ziplist和linkedlist(adlist.c)实现的，3.2版本后引入了quicklist的数据结构，列表底层都由quicklist来实现。本质上，quicklist是由ziplist和linkedlist结合而成。所以，学习quicklist之前，需要明白[ziplist](../redis-parse-zskiplist)的原理。  
<!-- more -->
**ziplist：**  
优点：存储效率高，内存开销少。  
